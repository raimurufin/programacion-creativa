...
int alto, ancho,largo;
  ...
  Figura(float x, float y,float r,int red,int green,int blue,float posX, float posY,float x1,float y1, int ancho, int alto, int largo, float r2){//constructor
    this.x = x;
    this.y = y;
    this.r = r;
    this.red=red;
  ...
  void circulo1(){
    ...
    for(int i = 0 ; i< 50; i++){// 50 arcos
      strokeWeight(5);//ancho del borde
      stroke(20);//color del contorno
      pushMatrix();
      noFill();//sin relleno
      r = r + 0.001;
      rotate(radians(5*i)*r);//rota al rededor del eje central y varia segun valor de r y de i
...
void circulos(){
  ...
  for(int l =0; l<8;l++){//aumenta la cantidad de"lineas"en 8 cantidades
    for(int o = 0; o < 15; o++){//se forman "lineas" de circulos,15 circulos
  ...
  /*Circulos repetidos de color azul rodeando una zona alrededor del centro*/
    pushMatrix();
      ...
      for (int p = 0; p < 60; p++) {// cantidad de lineas de circulos repetidos
        for (int u=0; u < 5; u++) {// numero de circulos por linea
